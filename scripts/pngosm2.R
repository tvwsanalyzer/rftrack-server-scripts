# -------------------------------------------------------------------------------
# Generate png for film2 with R and OpenStreetMap
# -------------------------------------------------------------------------------
# see:
# http://tuxette.nathalievilla.org/?p=1152
#
...
map_g_str <- 
	openmap(c(43.611522,1.428533), c(43.598903,1.461059),type=type.map[1])
# 					location = bbox,
					location = c(lon = tbl$lon[i], tbl$lat[i]),
#					location = c(lon = mean(lon), lat = mean(lat)),
#					zoom = mrz, 
...
###	)
	...
	# ori: toString(i)
fname <- paste("./video/maps2/",str_pad(i, 6, pad = "0"),"_map.png")
